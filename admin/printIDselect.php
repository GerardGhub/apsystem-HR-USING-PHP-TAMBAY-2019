...
<center class="apam"><a href="" id="printpagebutton" onclick="myFunction()">Click to Print following ID</a></center>
...
<table border="1" class="table table-hover" align="center" cellpadding="0" cellspacing="0" style="width:100%">
   <?php
	...
	$get_id = $_GET['id'];
          $query = mysql_query("SELECT *, employees.id AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id LEFT JOIN schedules ON schedules.id=employees.schedule_id LEFT JOIN department ON department.id=employees.department LEFT JOIN employee_status ON employee_status.id=employees.emp_status WHERE employees.resigned='1' AND employees.selected='selected' LIMIT 2");                //carreer oportunity  we are infinite so gave me some sunshine!@!@!@!@!@!@!
  ...
  <div class="column" style="background-color:window;">
    <h5>				  
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/GerardSs.jpg" width="100" height="65" /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<u>'.$column['employee_id'].'</u></h5>
    <!--p>Some text..</p-->
				  				  <br>
...
<!--  this is the first second phase 2 -->
	  ...
	  $start=4;
	  $limit=2;
          $query = mysql_query("SELECT *, employees.id AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id LEFT JOIN schedules ON schedules.id=employees.schedule_id LEFT JOIN department ON department.id=employees.department LEFT JOIN employee_status ON employee_status.id=employees.emp_status WHERE employees.resigned='1' AND employees.selected='selected' LIMIT 2,2");                
            while($row = mysql_fetch_array($query))
              {
  ...
  <div class="column" style="background-color:window;">
    <h5>				  
				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/GerardSs.jpg" width="100" height="65" /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<u>'.$row['employee_id'].'</u></h5>
    <!--p>Some text..</p-->
				  				  <br>
...
<br>&nbsp;&nbsp;<emp style="font-size:12px;">'.$column['date_post'].'&nbsp;To&nbsp; '.$today.'</emp> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src = "BCG/html/image.php?filetype=PNG&dpi=72&scale=1&rotation=0&font_family=Arial.ttf&font_size=9&height=10&text='.$column['code2'].'&thickness=31&start=NULL&code=BCGcode128" /><br> <span style="bottom: 1px;">&nbsp;'.$column['item_names'].'</td>';
	...
	<!-- phase 4 barcode -->
	  ...
	  $start=12;
	  $limit=4;
          $query = mysql_query("SELECT *, employees.id AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id LEFT JOIN schedules ON schedules.id=employees.schedule_id LEFT JOIN department ON department.id=employees.department LEFT JOIN employee_status ON employee_status.id=employees.emp_status WHERE employees.resigned='1' AND employees.selected='selected' LIMIT 4,2");                
            while($row = mysql_fetch_array($query))
              {
                $item=$row['item_category'];
				$today = date("m j, Y");
...
';
	...
	<!-- phase 5 of the barcode -->
	  ...
	  $start=16;
	  $limit=4;
          $query = mysql_query("SELECT *, employees.id AS empid FROM employees LEFT JOIN position ON position.id=employees.position_id LEFT JOIN schedules ON schedules.id=employees.schedule_id LEFT JOIN department ON department.id=employees.department LEFT JOIN employee_status ON employee_status.id=employees.emp_status WHERE employees.resigned='1' AND employees.selected='selected' LIMIT 6,2");                
            while($row = mysql_fetch_array($query))
              {
                $item=$row['item_category'];
				$today = date("m j, Y");
