            ...
            <div class="box-body">
             <table id="example1" class="table table-bordered">
				...
				//  SELECT *, attendance.employee_id AS attid, employees.id AS empid FROM employees LEFT JOIN attendance ON //attendance.employee_id=employees.id WHERE attendance.date ='2019-10-31' IS NULL
				  	...
				  	$date_now = date('Y-m-d');
                    $sql = "SELECT *, employees.employee_id AS empid, attendance.id AS attid FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id WHERE date BETWEEN '$from' AND '$to'";
                    $query = $conn->query($sql);
