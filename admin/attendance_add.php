<?php
	...
	if(isset($_POST['add'])){
		...
		$failed_to_swiped = $_POST['failed_to_swiped'];
		$attendance_department = $_POST['attendance_department'];
		...
		if($query->num_rows < 1){
			...
			if($query->num_rows > 0){
				...
				//
				$sql = "INSERT INTO attendance (employee_id, date, time_in, time_out, status,adjusted,failed_to_swiped,attendance_department) VALUES ('$emp', '$date', '$time_in', '$time_out', '$logstatus','1','$failed_to_swiped','$attendance_department')";
				if($conn->query($sql)){
	...
	header('location: viewdepdtr.php');
